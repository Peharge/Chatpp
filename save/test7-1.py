...
class ChatApp(QWidget):
    ...
    def initUI(self):
        ...
        # Pfad zur SVG-Datei
        icon_pathwe = 'C:\\Users\\julia\\PycharmProjects\\test23\\web-img.svg'

        web_button = QPushButton()

        # Icon laden und auf die gewünschte Größe skalierens
        icon_pixmap = QPixmap(icon_pathwe).scaled(70, 70)
        icon = QIcon(icon_pixmap)
        web_button.setIcon(icon)

        # Hinzufügen eines Schatteneffekts zum Button
        shadow_effect = QGraphicsDropShadowEffect(web_button)
        shadow_effect.setBlurRadius(10)
        shadow_effect.setColor(Qt.black)  # Schwarze Farbe für den Schatten
        shadow_effect.setOffset(5, 5)
        web_button.setGraphicsEffect(shadow_effect)
        web_button.setFixedSize(75, 35)
        web_button.setIconSize(QtCore.QSize(18, 18))  # Hier Größe der SVG-Datei angeben
        web_button.setIconSize(QSize(18, 18))  # Hier Größe der SsVG-Datei angeben
        web_button.setFont(QFont('Arial', 12, QFont.Bold))
        ...
        web_button.clicked.connect(self.vc)

        def move_web_button_up():
            web_button.move(web_button.x(), web_button.y() - 5)
        ...
        # Verbinde die Funktionen mit den entsprechenden Signalen.
        web_button.pressed.connect(move_web_button_up)
        web_button.released.connect(lambda: move_web_button_down(web_button))

        # Verbinde die enterEvent und leaveEvent mit den Funktionen.
        web_button.enterEvent = lambda event: move_web_button_up()
        web_button.leaveEvent = lambda event: move_web_button_down(web_button)
        ...
        """)
        # Füge den special_char_button zum Layout hinzu mit einem Abstand von 20px oben.
        button_layout.addWidget(self.setting_button, alignment=Qt.AlignLeft)
